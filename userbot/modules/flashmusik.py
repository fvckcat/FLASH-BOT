...
async def _(event):
    ...
    if event.pattern_match.group(1):
        ...
        query = reply.message
        await event.edit("`Musik Telah Ditemukan, Sedang Mengunggah...`")
    else:
...
async def _(event):
    ...
    if l:
        await event.edit("`Vidio Berhasil Ditemukan.`")
    else:
...
CMD_HELP.update(
    {
        "musik": ">`.net <Artis - Judul Lagu>`"
        "\nFungsi: Download Musik Dari @WooHaiBot"
        "\n\n>`.net now`"
        "\nFungsi: Download LastFM scrobble."
        "\n\n>`.vsong <Artis - Judul Lagu>`"
        "\nFungsi: Menemukan dan mengunggah video clip."
        "\n\n>`.smd <Artis - Judul Lagu>`"
        "\nFungsi: Download musik dari Spotify"
        "\n\n>`.deez (spotify/link deezer)`"
        "\nFungsi: Download musik dari deezer."
