...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response.text.startswith("I understand only stickers"):
            await event.edit("`Maaf Kawan, Saya Tidak Bisa Mengubah Ini Menjadi Gambar, Periksa Kembali Apakah Itu Sticker Animasi?`")
        else:
            response = conv.wait_event(
                events.NewMessage(
                    incoming=True,
...
async def sticker_to_png(sticker):
    ...
    if not img.document:
        await sticker.edit("`Maaf Kawan, Ini Bukan Sticker`")
        return False
